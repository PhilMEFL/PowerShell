...
"Port : $($printer.PortName) - IP : $($printer.IPAddress)"
CreatePrinterPort $printer.PortName $printer.IPAddress
#CreatePrinter $server $printer.Driver $printer.IPPort $printer.Sharename $printer.Location $printer.Comment $printer.Printername
}
